...
HRESULT InitialiseGraphics()
{
	...
	g_pModel0 = new Model(g_pD3DDevice, g_pImmediateContext);
	g_pModel0->LoadObjModel((char*)"assets/sphere.obj");

	g_pModel1 = new Model(g_pD3DDevice, g_pImmediateContext);
	g_pModel1->LoadObjModel((char*)"assets/cube.obj");
	g_pModel1->setPosZ(15);
	g_pModel1->setPosX(5);
...
void RenderFrame(void)
{
	...
	// RENDER HERE
	g_directional_light_shines_from = XMVector3Rotate(g_directional_light_shines_from, XMVectorSet(0.0000005* (180/XM_PI), 0.0000001 * (180 / XM_PI),0, 1.0f));

	g_directional_light_shines_from		= XMVector4Normalize(g_directional_light_shines_from);
	g_directional_light_colour			= XMVectorSet(0.8f, 0.8f, 0.8f, 0.0f); // green
	g_ambient_light_colour				= XMVectorSet(0.05f, 0.05f, 0.25f, 1.0f); // dark grey - use a small value for ambient
	...
	g_pModel0->LookAt(g_pModel1->getPosX(), g_pModel1->getPosZ(), g_pModel1->getPosY());
	g_pModel0->MoveForward(0.00005f);
	if (g_pModel0->CheckCollision(g_pModel1))
	{
		g_pModel0->MoveForward(-0.00005f);
	}
	g_pModel0->setDirectionalLight(g_directional_light_shines_from, g_directional_light_colour);
	g_pModel0->setAmbientLight(g_ambient_light_colour);
	g_pModel0->Draw(&view, &projection);


	g_pModel1->LookAt(g_pModel0->getPosX(), g_pModel0->getPosZ(), g_pModel0->getPosY());
	g_pModel1->MoveForward(0.0001f);
	if (g_pModel1->CheckCollision(g_pModel0))
	{
		g_pModel1->MoveForward(-0.0001f);
	}
	g_pModel1->setDirectionalLight(g_directional_light_shines_from, g_directional_light_colour);
	g_pModel1->setAmbientLight(g_ambient_light_colour);
